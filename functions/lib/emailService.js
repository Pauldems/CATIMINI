...
exports.sendVerificationEmail = (0, https_1.onCall)({
    ...
    try {
        ...
        if (!email || !displayName) {
            throw new https_1.HttpsError('invalid-argument', 'Email et nom requis');
        }
        // Configurer Resend
        const apiKey = process.env.RESEND_API_KEY;
        if (!apiKey) {
            throw new https_1.HttpsError('failed-precondition', 'Clé API Resend manquante');
        }
        ...
        const verificationLink = await (0, auth_1.getAuth)().generateEmailVerificationLink(email, actionCodeSettings);
      ...
      `;
        ...
        if (error) {
            throw new https_1.HttpsError('internal', `Erreur Resend: ${error.message}`);
        }
        return {
            success: true,
        ...
        throw new https_1.HttpsError('internal', 'Erreur lors de l\'envoi de l\'email');
    }
});
